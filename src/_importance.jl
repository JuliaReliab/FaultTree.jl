...
"""
function c1meas(ft::FTree, top::AbstractFTObject, env::Dict{Symbol,Tv}) where Tv <: Number
    f = ftbdd!(ft, top)
    bddcache = Dict()
...
"""
function c0meas(ft::FTree, top::AbstractFTObject, env::Dict{Symbol,Tv}) where Tv <: Number
    f = ftbdd!(ft, top)
    bddcache = Dict()
    p = prob(ft, f, env, bddcache=bddcache)
    b = grad(ft, f, env, bddcache=bddcache)
